...
- **ProductService:** Вся бизнес логика вынесена из контроллера в отдельный сервис. Валидация с помощью `FormRequest`. Для более безопасной валидации в будушем можно использовать `DTO`. Также в будущем, когда условий для фильтрации станет много можно прибегнуть к паттерну Pipeline.
...
```http
GET /api/products?q=iphone&price_from=1000&price_to=5000&category_id=2&in_stock=true&rating_from=4.5&sort=price_asc
